...
import gsv.{type FieldSeparator, type LineEnding, Unix, Windows}
...
pub fn dicts_with_missing_values_test() {
  ...
  gsv.from_dicts(data, gsv.Comma, gsv.Unix)
  |> should.equal(
...
pub fn quotes_test() {
  ...
  gsv.to_lists(stringy, gsv.Comma)
  |> should.be_ok
  |> should.equal([
    ["Date", "Type", "Price", "Ammount"],
    ["11/11/2024", "Apples", "7", "5"],
...
pub fn encode_test() {
  "Ben, 25
Austin, 21"
  |> test_lists_roundtrip(gsv.Comma, Unix)
}
...
Austin, 25, FALSE"
  ...
  rows
  |> gsv.from_lists(separator: gsv.Comma, line_ending: Windows)
  |> string.replace(each: "\"", with: "'")
  |> should.equal(
    "Ben, 25,' TRUE\n\r'' '\r
Austin, 25, FALSE\r\n",
...
fn test_lists_roundtrip(
  ...
  let assert Ok(parsed) = gsv.to_lists(input, gsv.Comma)
...
fn pretty_print_error(input: String) -> String {
  let assert Error(error) = gsv.to_lists(input, gsv.Comma)
